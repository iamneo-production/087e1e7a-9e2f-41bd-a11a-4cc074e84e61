...
const id = props.themeid;
  axios.put('https://8080-affdbaabdcabfabadfbbdfdacbcefeddcbcbaffb.project.examly.io/admin/editTheme/'+id,data)
  .then(response => {
...
const Editthemes = (props) => {
  ...
  const handleEdit = () => {
    ...
    axios.put('https://8080-dafbecdaebfdaaaabadfbbdfdacbcefeddcbcbaffb.project.examly.io/admin/editTheme/' + theme.themeId, data)
      .then(response => {
        if (response.data === "Theme edited") {
          toast.success("Theme Edited");
          handleClose();
      ...
      <Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Group className="mb-3">
              <Form.Control type="text" placeholder="Enter the theme name" id="enterThemeName" value={EditedThemeName}
                onChange={(e) => setEditedThemeName(e.target.value)} />
              {nameError && <Form.Text className="text-danger">{nameError}</Form.Text>}
            </Form.Group>
            <label>Theme Price:</label>
            <Form.Group className="mb-3">
              <Form.Control type="number" placeholder="Enter the theme price" id="enterThemePrice" value={EditedThemePrice}
                onChange={(e) => setEditedThemePrice(e.target.value)} />
              {priceError && <Form.Text className="text-danger">{priceError}</Form.Text>}
            </Form.Group>
            <label>Theme Description:</label>
            <Form.Group className="mb-3">
              <Form.Control type="text" placeholder="Enter the theme description" id="enterThemeDescription" value={EditedThemedescription}
                onChange={(e) => setEditedThemedescription(e.target.value)} />
